...
/* 			jsonParam["bqm.time"]= $("[name='bqm.time']").val();
			...
			$tt = $(this).parent().parent();//当前行
			
			$.post("bill_ajaxGetBillGoodsDetail.action",jsonParam,function(data){
				//将制定标记tr删除
				$('.ajaxMsg').empty();
				...
				var sum=0;
				for(i = 0;i<billGoodsList.length;i++){
					var $tr = $("<tr align='center' class='ajaxMsg'></tr>");
					//共计5列
					var $td1 = $("<td height='30'>"+billGoodsList[i].order.orderNum+"</td>");
					$tr.append($td1);
					var $td2 = $("<td>"+billGoodsList[i].order.createTimeView+"</td>");
					$tr.append($td2);
					var $td3 = $("<td>"+billGoodsList[i].num+"</td>");
					$tr.append($td3);
					var $td4 = $("<td align='right'>"+billGoodsList[i].priceView+"&nbsp;元</td>");
					$tr.append($td4);
					var orderDetailSum=billGoodsList[i].num*billGoodsList[i].priceView;
					var $td5 = $("<td align='right'>"+intToFloat(orderDetailSum)+"&nbsp;元</td>");
					$tr.append($td5);
					sum+=orderDetailSum;
					$tt.after($tr);
					$tt=$tr;
			...
			<div class="square-o-top">
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td width="70">订单类别:</td>
						<td width="190">
							<s:select name="bqm.type" list="@cn.brotherChun.erp.invoice.order.vo.OrderModel@buyMap" headerKey="-1" headerValue="----请-选-择----" />
						</td>
						<td width="70">开始日期:</td>
						<td width="190">
					...
					<tr>
						...
						<td>厂商名称:</td>
						<td>
							<s:select name="bqm.supplierUuid" list="supplierList" listKey="uuid" listValue="name" cssClass="kuan" headerKey="-1" headerValue="----请-选-择----"/>
						</td>
						<td>结束日期:</td>
						<td width="190">
							<input type="text" size="18" onfocus="c.showMoreDay=false;c.show(this);" />
